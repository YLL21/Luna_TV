  ...
  return (
    ...
    >
      <div className='relative'>
        {/* 发光背景效果 */}
        <div className='absolute inset-0 bg-green-400 opacity-0 group-hover:opacity-20 blur-xl transition-opacity duration-300 rounded-lg scale-110'></div>

        <span className='relative text-2xl font-bold bg-green-600 dark:bg-green-400 bg-clip-text text-transparent tracking-tight transition-all duration-300 group-hover:scale-105 inline-block group-hover:drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]'>
          {siteName}
        </span>
      </div>
    </Link>
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      ...
      <div className='hidden md:flex'>
        ...
        >
          {/* 装饰性背景光晕 */}
          <div className='absolute top-0 left-0 w-full h-32 bg-green-400/5 pointer-events-none'></div>
          <div className='absolute bottom-0 left-0 w-full h-48 bg-purple-400/5 pointer-events-none'></div>
          ...
          <div className='flex h-full flex-col relative z-10'>
            ...
            <div className='relative h-16'>
              ...
              >
                {/* 渐变边框效果 */}
                <div className='absolute inset-0 rounded-full bg-green-400 opacity-0 group-hover/toggle:opacity-100 transition-opacity duration-300 animate-pulse'></div>
            ...
            <nav className='px-2 mt-4 space-y-1'>
              <Link
                ...
                onClick={() => setActive('/')}
                data-active={active === '/'}
                className={`group relative flex items-center rounded-lg px-2 py-2 pl-4 text-gray-700 hover:bg-green-50 hover:text-green-600 data-[active=true]:bg-green-500/20 data-[active=true]:text-green-700 font-medium transition-all duration-200 min-h-[40px] dark:text-gray-300 dark:hover:bg-green-500/10 dark:hover:text-green-400 dark:data-[active=true]:bg-green-500/15 dark:data-[active=true]:text-green-400${isCollapsed ? 'w-full max-w-none mx-0' : 'mx-0'
                  } gap-3 justify-start data-[active=true]:shadow-lg data-[active=true]:shadow-green-500/20`}
              >
                ...
                {/* 激活状态的左侧边框指示器 */}
                <div className='absolute left-0 top-1/2 -translate-y-1/2 w-1 h-0 bg-green-500 rounded-r-full transition-all duration-200 data-[active=true]:h-8 opacity-0 data-[active=true]:opacity-100' data-active={active === '/'}></div>
              </Link>
              <Link
                href='/search'
                onClick={(e) => {
                ...
                className={`group relative flex items-center rounded-lg px-2 py-2 pl-4 text-gray-700 hover:bg-blue-50 hover:text-blue-600 data-[active=true]:bg-blue-500/20 data-[active=true]:text-blue-700 font-medium transition-all duration-200 min-h-[40px] dark:text-gray-300 dark:hover:bg-blue-500/10 dark:hover:text-blue-400 dark:data-[active=true]:bg-blue-500/15 dark:data-[active=true]:text-blue-400${isCollapsed ? 'w-full max-w-none mx-0' : 'mx-0'
                  } gap-3 justify-start data-[active=true]:shadow-lg data-[active=true]:shadow-blue-500/20`}
              >
                <div className='w-4 h-4 flex items-center justify-center relative z-10'>
                ...
                {/* 激活状态的左侧边框指示器 */}
                <div className='absolute left-0 top-1/2 -translate-y-1/2 w-1 h-0 bg-blue-500 rounded-r-full transition-all duration-200 data-[active=true]:h-8 opacity-0 data-[active=true]:opacity-100' data-active={active === '/search'}></div>
              </Link>
            </nav>
                ...
                {menuItems.map((item, index) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`group relative flex items-center rounded-lg px-2 py-2 pl-4 text-sm text-gray-700${theme.hover} ${theme.active} ${theme.text} transition-all duration-200 min-h-[40px] dark:text-gray-300 ${isCollapsed ? 'w-full max-w-none mx-0' : 'mx-0'
                        } gap-3 justify-start ${theme.shadow} animate-[slideInFromLeft_0.3s_ease-out] opacity-0`}
                      style={{
                    ...
                    >
                      ...
                      {/* 激活状态的左侧边框指示器 */}
                      <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-0${theme.border} rounded-r-full transition-all duration-200 data-[active=true]:h-8 opacity-0 data-[active=true]:opacity-100`} data-active={isActive}></div>
                    </Link>
                  );
                })}
              </div>
